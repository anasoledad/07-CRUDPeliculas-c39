...
function validarURLImagen(value){
    ...
    if(patron.test(value)){
        console.log('formato correcto de url');
        return true;
    }else{
        console.log('no es el formato de una url');
        return false;
    }
}
...
export function sumarioValidaciones(titulo, descripcion, imagen, duracion){
    ...
    if(!cantidadCaracteres(titulo,3,100)){
        resumen += 'Corregir el campo del titulo debe contener entre 3 y 100 caractes <br>';
    }
    ...
    if(!validarDuracion(duracion)){
        resumen += 'Corregir la duración, debe ser un numero de 3 digitos como maximo <br>'
    }
